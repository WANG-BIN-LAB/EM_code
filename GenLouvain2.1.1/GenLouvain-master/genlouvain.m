...
%使用模块化/质量矩阵B实现了一种类似louvaine的贪婪社区检测方法，该方法对质量函数Q进行编码，定义为将所有元素B(i,j)相加，使节点i和j位于同一个社区。
...
%算法分两个阶段迭代进行:每次移动一个节点来优化质量，直到没有移动节点来提高质量;然后聚合到该点的社区，以构建一个新的网络，其中每个节点代表一个社区。
%输出向量S对获得的社区分配进行编码，S(i)标识分配给节点i的社区。输出Q给出了网络结果分区的质量。
...
%对于小型网络，通过处理全模块化/质量矩阵B = A - k'*k/twom，而不是函数句柄；可以更简单地获得合理有效的结果;
%预期的用途还包括Mucha等人2010年的“多层”网络质量函数，其中B将交互编码为等效矩阵(参见网上发布的示例http://netwiki.amath.unc.edu/GenLouvain)。
%   Notes:
%
...
%输出Q提供了B的适当元素的和，而不需要任何重新缩放。同样，在上面的例子中，我们将Q重新调整了2m = sum(k)，使Q <= 1。
...
%initialise variables and do symmetry check初始化变量并检查对称性
if isa(B,'function_handle')%判断B是否为函数句柄，如果是，返回逻辑1（真）；如果不是，返回逻辑0
    n=length(B(1));
    S=(1:n)';
    if isempty(S0)
        S0=(1:n)';
...
dtot=0; %keeps track of total change in modularity跟踪模块化的总体变化
y = S0;%刚开始时每个时间窗里每个节点是一个社区
%Run using function handle, if provided
while (isa(M,'function_handle')) %loop around each "pass" (in language of Blondel et al) with B function handle  B不是函数句柄跳过这一步
    clocktime=clock;
...
while ~isequal(Sb,S2) %loop around each "pass" (in language of Blondel et al) with B matrix    
    ...
    while ~isequal(yb,y)
        dstep=1;
        while (~isequal(yb,y)) && (dstep/dtot>2*eps) && (dstep>10*eps) %This is the loop around Blondel et al's "first phase"第一阶段
            yb = y;
            dstep=0;
            group_handler('assign',y);
            for i = myord(length(M))
                di=group_handler(movefunction,i,M(:,i));%每次取一层中一个节点与其他节点层内Q值，以及它与相邻层的关系
...
%Computes new aggregated network (communities --> nodes)计算新的聚合网络(社区——>节点)
...
%ith column of metanetwork (used to create function handle)metanetwork的第i列(用于创建函数句柄)
